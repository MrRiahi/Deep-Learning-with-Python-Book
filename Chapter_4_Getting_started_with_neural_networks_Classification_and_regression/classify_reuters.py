...
def get_imdb_dataset(num_words=10000):
    ...
    (train_data, train_labels), (test_data, test_labels) = tf.keras.datasets.reuters.\
...
def to_one_hot_encoding(labels, dimensions=46):
    """
    ...
    one_hot_labels = np.zeros(shape=(len(labels), dimensions))
...
def build_model(input_shape):
    ...
    X_output = tf.keras.layers.Dense(units=46, activation='softmax')(X)
...
y_train = to_one_hot_encoding(labels=train_labels, dimensions=46)
y_val = to_one_hot_encoding(labels=val_labels, dimensions=46)
y_test = to_one_hot_encoding(labels=test_labels, dimensions=46)
